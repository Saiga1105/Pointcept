...
class ThreeDomFeaturesDataset(Dataset):
    def __init__(
        self,
        split="train",
        data_root="data/3dom",
        ...
        super(ThreeDomFeaturesDataset, self).__init__()
    ...
    def get_data(self, idx):
        ...
        coord = data["coord"]
        verticality = data["verticality"].reshape([-1, 1])
        normal = data["normal"]
        if "semantic_gt" in data.keys():
            segment = data["semantic_gt"].reshape([-1])
        else:
            segment = np.ones(coord.shape[0]) * -1
        data_dict = dict(coord=coord, normal=normal, verticality=verticality, segment=segment)
        return data_dict
